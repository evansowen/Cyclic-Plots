...
names(df)[5] <- "Stress"
df$Strain <- (df$TKS[1]-df$TKS)/df$TKS[1]
...
no.cycles <- length(which(low.lengths$cycle.length<10))
...
low.lengths <- filter(low.lengths, cycle.length < 10)
low.lengths$index <- floor(low.lengths$index)
...
for (k in 1:((no.cycles-1)/5)) {
  plotnames[k] <- paste0("plot", k, ".png")
  ggsave(plotnames[k], myplots[[5*k]], device = 'png', dpi = 100)
}
...
gifski(png_files, gif_file = "animation.gif", width = 800, height = 550, delay = 0.15)
